...
func TestUpdate(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			...
			if tC.wantOSInstall {
				osDownload := []byte("os")
				osUnpacked := []byte("os unpacked")
				remote.EXPECT().GetOS().Return(osDownload, nil)
				verifier.EXPECT().VerifyAndUnpack(gomock.Eq(osDownload)).Return(osUnpacked, nil)
				local.EXPECT().InstallOS(gomock.Eq(osUnpacked)).Return(nil)
			}
			if tC.wantAppInstall {
				appDownload := []byte("applet")
				appUnpacked := []byte("applet unpacked")
				remote.EXPECT().GetApplet().Return(appDownload, nil)
				verifier.EXPECT().VerifyAndUnpack(gomock.Eq(appDownload)).Return(appUnpacked, nil)
				local.EXPECT().InstallApplet(gomock.Eq(appUnpacked)).Return(nil)
			}
			reboot, err := updater.Update(ctx)
			if err != nil {
				t.Errorf("Update(): %v", err)
			}
			if tC.wantReboot != reboot {
				t.Errorf("reboot: got != want (%t != %t)", reboot, tC.wantReboot)
			}
		})
	}
}
